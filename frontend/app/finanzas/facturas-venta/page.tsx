...
export default function FacturasVentaPage() {
  ...
  const [sortKey, setSortKey] = useState<
    'fecha' | 'monto_total' | 'cliente_nombre' | 'cliente_rut' | 'documento_numero'
  >('fecha');
          ...
          <button
            ...
            onClick={(e) => {
              ...
              if (newKey && !linksByKey[newKey]) {
                ...
                fetchReconcileLinks({ sales_doc: row.documento_numero, sales_date: row.fecha })
                  .then((res) =>
                    setLinksByKey((prev) => ({
                      ...prev,
  ...
  async function load() {
    try {
      setLoading(true);
      setError(null);
      const res: PagedResponse<VentaRow> = await fetchInvoicesVenta({
        page,
        page_size: pageSize,
        order_by: sortKey,
      ...
      setError(e?.message || 'Error cargando facturas de venta');
    } finally {
      setLoading(false);
    }
  }
  ...
  const onSort = (key: string, direction: 'asc' | 'desc') => {
    // Only allow known sortable keys
    if (
      ['fecha', 'monto_total', 'cliente_nombre', 'cliente_rut', 'documento_numero'].includes(key)
    ) {
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400">Fuente canónica: v_facturas_venta</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          <input
            ...
            placeholder="Buscar (doc o cliente)"
      ...
      <DataTable
        ...
        loading={loading}
        emptyMessage="Sin facturas de venta"
        isRowExpanded={(row: VentaRow) => expandedKey === `${row.documento_numero}|${row.fecha}`}
        renderExpandedRow={(row: VentaRow) => {
          const key = `${row.documento_numero}|${row.fecha}`;
                ...
                {entry.items.map((it, i) => (
                  <li key={i} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs ${it.type === 'bank' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-200' : it.type === 'purchase' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-200' : 'bg-slate-100 text-slate-700 dark:bg-slate-900/40 dark:text-slate-200'}`}
                      >
                        {it.type}
                      </span>
